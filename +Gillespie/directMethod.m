function [ t, x, result ] = directMethod( stoich_matrix, propensity_fcn, tspan, x0,...
                                  params, notetimes, output_fcn, MAX_OUTPUT_LENGTH)
...
%counter = 0;
nt_count = 1;
result = zeros(num_species, size(notetimes,2));

%% MAIN LOOP
while T(rxn_count) < tspan(2) 
    %counter = counter+1;
    if T(rxn_count) > notetimes(nt_count)
        result(:,nt_count) = X(rxn_count-1,:);
        if nt_count < size(notetimes,2)
            nt_count = nt_count + 1;
        end
    end
    ...
    %{
    if counter==100000
        append("tau= ",string(tau))
        append("mu= ", string(mu))
        t_test = T(rxn_count)
        x_test = X(rxn_count,:)
        %append("t= ",string(t_test))
        %append("x= ",string(x_test))
        %counter = 0;
    end
    if counter==100001
        append("tau= ",string(tau))
